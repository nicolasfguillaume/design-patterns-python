...
class BankAccount:
    def __init__(self, balance=0):
        self.balance = balance
        # store every single state of the system as a memento
        self.changes = [Memento(self.balance)]
        # position of the current state in the self.changes list
        # so we can do undo (going backward) and redo (going forward)
        self.current = 0
